...
void serialdata( unsigned char serial)
{
	while((U0LSR & 0x20)==0);
	U0THR=serial;
}
void serial(void)
{
	...
	while(1)
	{
		input_val=FIO4PIN;
		FIO3PIN = FIO4PIN;
		a=(unsigned char )input_val / 100;
		b=(unsigned char )input_val % 100;
		c=(unsigned char)b%10;
...
int main ()
{
	...
	// Since we know most of the things apriori, we will not make a generic
	// sine wave generator, but a specific and blunt sine wave generator.
	// Further, since we are very greedy, we will make a 0.4sin(wt), which
	// will ensure that the values don't cross 255. Hence we can replace
	// ldr with mov, which takes only one cycle.
	
	/*
		Calculations:
		Each MOV instruction takes 1 cycle. Each STR instruction takes
		2 cycles. There are 13 transfers. Hence there are totally 39 cycles
		per a sine cycle and possible atmost 4 cycles for branching. Hence,
		the total time per a sine cycle = 43 * cycle time
	*/
